...
class TaskManager:
    ...
    def update(self):
        ...
        pd.read_csv(f"tasks/{self._specific_date()}.csv", usecols=["Group", "Task", "State"]).to_csv(
            f"tasks/{self._specific_date(days=-1)}.csv", index=False
        )
        print("")
    ...
    def _show_tasks(self, num=None):
        ...
        for group in groups:
            print(f"---  {group}  ---")
            print("")
            for task, cnum in zip(
                task_df[task_df["Group"] == group].loc[:, "Task"], task_df[task_df["Group"] == group].loc[:, "State"]
            ):
                if num is None:
